...
Perceptron::Perceptron(float thresholdIn)
{
	nrOfInputs = 0;
...
bool Perceptron::training(std::vector<std::vector<float>>& trainingData, std::vector<float>& outputs, long nrIterations, float lms, float learningRate)
{
	...
	for ( long i = 0; i < nrIterations; ++i )
	{
		...
		for ( int j = 0; j < trainingData.size(); ++j )
		{
			...
			if ( fabs(delta) >= 0.0001  )
			{
				...
				{
					weights[k] = weights[k] + learningRate*(trainingData[j][k])*delta;
				}
			}
		}
		for ( int j = 0; j < trainingData.size(); ++j )
